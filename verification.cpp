...
public:
    ...
    __always_inline void constructor(bytes name, bytes symbol, uint8 decimals, uint256 root_public_key,
                                     cell wallet_code, bytes proof_msg_bytes) {
        name_ = name;
        symbol_ = symbol;
        decimals_ = decimals;
    ...
    __always_inline lazy<MsgAddressInt>
        deployWallet(int8 workchain_id, uint256 pubkey, TokenId tokenId, WalletGramsType grams) {
        ...
        dest_handle.deploy(wallet_init, Grams(grams.get())).call<&ITONTokenWallet::accept>(tokenId);
    ...
    __always_inline void grant(lazy<MsgAddressInt> dest, TokenId tokenId, WalletGramsType grams) {
        require(root_public_key_ == tvm_pubkey(), error_code::message_sender_is_not_my_owner);
    ...
    __always_inline TokenId mint(TokenId tokenId) {
        require(root_public_key_ == tvm_pubkey(), error_code::message_sender_is_not_my_owner);
    ...
    __always_inline lazy<MsgAddressInt> getWalletAddress(int8 workchain_id, uint256 pubkey) {
    ...
    __always_inline static int _on_bounced(cell msg, slice msg_body) {
        ...
        require(opt_hdr && opt_hdr->function_id == id_v<&ITONTokenWallet::accept>, error_code::wrong_bounced_header);
...
private:
    __always_inline std::pair<StateInit, lazy<MsgAddressInt>> calc_wallet_init(int8 workchain_id, uint256 pubkey) {
        DTONTokenWallet wallet_data {
            name_,        symbol_, decimals_, root_public_key_, pubkey, lazy<MsgAddressInt> {tvm_myaddr()},
            wallet_code_, {},      {}};
