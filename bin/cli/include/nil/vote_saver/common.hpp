...
struct marshaling_policy {
    ...
    // static void write_initial_phase_voter_data(const std::array<bool, encrypted_input_policy::public_key_bits>
    // &voter_pubkey,
    //                                            const std::array<bool, encrypted_input_policy::secret_key_bits>
    //                                            &voter_skey, std::size_t i, const std::string &voter_pk_out, const
    ...
    static void serialize_initial_phase_voter_data(
        const std::array<bool, encrypted_input_policy::public_key_bits> &voter_pubkey,
    ...
    //         const proving_key_type &pk_crs, const verification_key_type &vk_crs, const elgamal_public_key_type
    //         &pk_eid, const elgamal_private_key_type &sk_eid, const elgamal_verification_key_type &vk_eid, const
    //         primary_input_type &eid, const primary_input_type &rt, const std::vector<std::array<bool,
    //         encrypted_input_policy::merkle_hash_type::digest_bits>> hashes, const std::string &r1cs_proving_key_out,
    //         const std::string &r1cs_verification_key_out, const std::string &public_key_output,
    //         const std::string &secret_key_output, const std::string &verification_key_output, const std::string
    //         &eid_output, const std::string &rt_output) {
    ...
    static void serialize_initial_phase_admin_keys(
        ...
        r1cs_proving_key_out = serialize_obj<r1cs_proving_key_marshalling_type>(
            pk_crs,
            std::function(nil::crypto3::marshalling::types::fill_r1cs_gg_ppzksnark_fast_proving_key<proving_key_type,
                                                                                                    endianness>));
    ...
    //             proof,
    //             std::function(nil::crypto3::marshalling::types::fill_r1cs_gg_ppzksnark_proof<proof_type,
    //             endianness>));
    ...
    //             pk_eid,
    //             std::function(nil::crypto3::marshalling::types::fill_public_key<elgamal_public_key_type,
    //             endianness>));
    ...
    static void serialize_data(std::size_t proof_idx, const proof_type &proof, const primary_input_type &pinput,
                               const encrypted_input_policy::encryption_scheme_type::cipher_type::first_type &ct,
                               const primary_input_type &sn, std::vector<std::uint8_t> &proof_blob,
    ...
    static scalar_field_value_type get_field_element_from_bits(const std::vector<bool> &bits) {
        ...
        for (std::size_t i = 0; i < bits.size(); ++i) {
            /* push in the new bit */
            const scalar_field_value_type v =
                (bits[bits.size() - 1 - i] ? scalar_field_value_type::one() : scalar_field_value_type::zero());
    ...
    static std::vector<std::uint8_t> serialize_bitarray(const std::array<bool, Bits> &bitarray) {
        ...
        nil::crypto3::detail::pack<nil::crypto3::stream_endian::big_octet_big_bit,
                                   nil::crypto3::stream_endian::big_octet_big_bit, 1, 8>(in.begin(), in.end(),
                                                                                         out.begin());
    ...
    static std::array<bool, Bits> deserialize_bitarray(InputIterator begin, InputIterator end) {
        constexpr int octets = Bits / 8 + (Bits % 8 ? 1 : 0);
        constexpr int bits_ceil = octets * 8;
        ...
        nil::crypto3::detail::pack<nil::crypto3::stream_endian::big_octet_big_bit,
                                   nil::crypto3::stream_endian::big_octet_big_bit, 8, 1>(in.begin(), in.end(),
                                                                                         out.begin());
    ...
    static containers::merkle_tree<encrypted_input_policy::merkle_hash_type, encrypted_input_policy::arity>
        deserialize_merkle_tree(std::size_t tree_depth, InputIterator first, InputIterator last) {
        std::size_t tree_length =
            containers::detail::merkle_tree_length(1 << tree_depth, encrypted_input_policy::arity);
        BOOST_ASSERT(std::distance(first, last) % tree_length == 0);
        std::size_t hash_octets = std::distance(first, last) / tree_length;

        std::vector<std::vector<bool>> hashes;
        hashes.reserve(tree_length);

        for (auto iter = first; iter < last; iter += hash_octets) {
            auto array =
                deserialize_bitarray<encrypted_input_policy::merkle_hash_type::digest_bits>(iter, iter + hash_octets);
            hashes.emplace_back(array.begin(), array.end());
        }
        BOOST_ASSERT(hashes.size() == tree_length);

        return containers::merkle_tree<encrypted_input_policy::merkle_hash_type, encrypted_input_policy::arity>(
    ...
    // static std::vector<std::array<bool, encrypted_input_policy::public_key_bits>> read_voters_public_keys(std::size_t
    // tree_depth,
    //                                                                                                       const
    //                                                                                                       std::string
    //                                                                                                       &voter_public_key_output)
    ...
    static std::vector<std::array<bool, encrypted_input_policy::public_key_bits>>
        deserialize_voters_public_keys(std::size_t tree_depth, InputIterator first, InputIterator last) {
        std::size_t participants_number = 1 << tree_depth;
        BOOST_ASSERT(std::distance(first, last) <= participants_number);
        std::vector<std::array<bool, encrypted_input_policy::public_key_bits>> result;

        for (auto itr = first; itr < last; ++itr) {
            result.emplace_back(
                deserialize_bitarray<encrypted_input_policy::public_key_bits>(itr->begin(), itr->end()));
        }

        for (auto i = std::distance(first, last); i < participants_number; i++) {
            result.emplace_back(std::array<bool, encrypted_input_policy::public_key_bits> {});
        }
    ...
    //             pk_eid_blob,
    //             std::function(nil::crypto3::marshalling::types::make_public_key<elgamal_public_key_type,
    //             endianness>));
    // }
    ...
    //             vk_eid_blob,
    //             std::function(
    //                     nil::crypto3::marshalling::types::make_verification_key<elgamal_verification_key_type,
    //                     endianness>));
    // }
    ...
    //             sk_eid_blob,
    //             std::function(nil::crypto3::marshalling::types::make_private_key<elgamal_private_key_type,
    //             endianness>));
    // }
    ...
    //             pk_crs_blob,
    //             std::function(
    //                     nil::crypto3::marshalling::types::make_r1cs_gg_ppzksnark_fast_proving_key<proving_key_type,
    //                     endianness>));
    // }
    ...
    static proving_key_type deserialize_pk_crs(const std::vector<std::uint8_t> &pk_crs_blob) {
        return deserialize_obj<r1cs_proving_key_marshalling_type, proving_key_type>(
            pk_crs_blob,
            std::function(nil::crypto3::marshalling::types::make_r1cs_gg_ppzksnark_fast_proving_key<proving_key_type,
                                                                                                    endianness>));
    }
    ...
    //             proof_blob,
    //             std::function(nil::crypto3::marshalling::types::make_r1cs_gg_ppzksnark_proof<proof_type,
    //             endianness>));
    // }
...
void process_encrypted_input_mode_init_admin_phase_generate_keys(std::size_t tree_depth, std::size_t eid_bits,
    ...
    std::size_t root_packed_size =
        (encrypted_input_policy::hash_component::digest_bits + (chunk_size - 1)) / chunk_size;
    ...
    components::multipacking_component<encrypted_input_policy::field_type> eid_packer(bp, eid_block.bits, eid_packed,
                                                                                      chunk_size);
    components::multipacking_component<encrypted_input_policy::field_type> sn_packer(bp, sn_digest.bits, sn_packed,
                                                                                     chunk_size);
    components::multipacking_component<encrypted_input_policy::field_type> root_packer(bp, root_digest.bits,
                                                                                       root_packed, chunk_size);
    logln("Variables number in the generated R1CS: ", bp.num_variables());
    ...
    logln(
        "Administrator generates private, public and verification keys for El-Gamal verifiable encryption "
        "scheme...");
    ...
    marshaling_policy::serialize_initial_phase_admin_keys(
        ...
        r1cs_proving_key_out, r1cs_verification_key_out, public_key_output, secret_key_output, verification_key_output);
    logln("Marshalling finished.");
}
...
void process_encrypted_input_mode_init_admin_phase_generate_data(
    ...
    auto public_keys = marshaling_policy::deserialize_voters_public_keys(tree_depth, public_keys_blobs.begin(),
                                                                         public_keys_blobs.end());
    ...
    std::vector<std::array<bool, encrypted_input_policy::merkle_hash_type::digest_bits>> hashes_array_vector(
        hashes_size, std::array<bool, encrypted_input_policy::merkle_hash_type::digest_bits> {});
    for (std::size_t i = 0; i < hashes_size; ++i) {
        std::copy_n(hashes[i].begin(), encrypted_input_policy::merkle_hash_type::digest_bits,
                    hashes_array_vector[i].begin());
    }
    logln("Administrator initial phase data marshalling started...");
    marshaling_policy::serialize_initial_phase_admin_data(eid_field, rt_field, hashes_array_vector, eid_output,
...
void process_encrypted_input_mode_vote_phase(
    std::size_t tree_depth, std::size_t eid_bits, std::size_t voter_idx, std::size_t vote,
    const std::vector<std::uint8_t> &merkle_tree_blob, const std::vector<std::uint8_t> &rt_blob,
    const std::vector<std::uint8_t> &eid_blob, const std::vector<std::uint8_t> &sk_blob,
    const std::vector<std::uint8_t> &pk_eid_blob, const std::vector<std::uint8_t> &proving_key_blob,
    const std::vector<std::uint8_t> &verification_key_blob, std::vector<std::uint8_t> &proof_blob,
    std::vector<std::uint8_t> &pinput_blob, std::vector<std::uint8_t> &ct_blob, std::vector<std::uint8_t> &sn_blob) {
    using scalar_field_value_type = typename encrypted_input_policy::pairing_curve_type::scalar_field_type::value_type;

    auto tree =
        marshaling_policy::deserialize_merkle_tree(tree_depth, merkle_tree_blob.begin(), merkle_tree_blob.end());
    ...
    auto sk = marshaling_policy::deserialize_bitarray<encrypted_input_policy::secret_key_bits>(sk_blob.begin(),
                                                                                               sk_blob.end());
    ...
    marshaling_policy::serialize_data(proof_idx, rerand_cipher_text.second,
...
void process_encrypted_input_mode_tally_admin_phase(std::size_t tree_depth,
    ...
    logln(
        "Administrator processes tally phase - aggregates encrypted ballots, decrypts aggregated ballot, "
...
bool process_encrypted_input_mode_tally_voter_phase(std::size_t tree_depth,
                                                    const std::vector<std::vector<std::uint8_t>> &cts_blobs,
                                                    const std::vector<std::uint8_t> &vk_eid_blob,
                                                    const std::vector<std::uint8_t> &pk_crs_blob,
                                                    const std::vector<std::uint8_t> &vk_crs_blob,
    ...
    logln(
        "Voter processes tally phase - aggregates encrypted ballots, verifies voting result using decryption "
        "proof...",
        "\n");
